...
blink_duration = 0.5  # Duration of the blink cycle
...
def drawPoint(x, y, color, blink_start_time):
    current_time = time.time()
    elapsed_time = current_time - blink_start_time
    if int(elapsed_time / blink_duration) % 2 == 0:
...
def drawScene():
    ...
    for point in points:
        drawPoint(point['x'], point['y'], point['color'], point['blink_start_time'])
